...
def numpy_to_cube(np_array, similar_cube, dimensions,type=''):
    """
    Function that converts a 1, 2, or 3 dimensional numpy array to a cube.
    (Inverse is array = cube.data)
    """
    from iris.coords import DimCoord
    l=[0.25+0.5*i for i in range(-180,180)]
    lo=[0.25+0.5*i for i in range(-360,360)]
    latitude = DimCoord(np.array(l),
                        standard_name='latitude',
                        units='degrees')
    longitude = DimCoord(np.array(lo),
                        standard_name='longitude',
                        units='degrees')
    new_cube = iris.cube.Cube.copy(similar_cube) # copy similar cube
    if type=='':
        new_cube=iris.cube.Cube(similar_cube.data,units='degrees',var_name=similar_cube.var_name,
    attributes=similar_cube.attributes,cell_methods=similar_cube.cell_methods,
    standard_name=similar_cube.standard_name,
    long_name=similar_cube.long_name,dim_coords_and_dims=[(latitude, 0),(longitude, 1)]) # copy similar cube
    # time, lat, lon
    if dimensions == 3:
...
def regrid_model(cube, regridcube):
    ...
    regridcube.coord('lat').standard_name = 'latitude'
    regridcube.coord('lon').standard_name = 'longitude'
