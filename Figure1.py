...
for rcp_option in range(0, rcp_options_length):
    ...
    for model_i in range(0, n_models_cmip5):
        ...
        rh_historical_cube_new = combine_netCDF_cmip5('cmip5_data/rh_Lmon_'+model+'_historical*', 'heterotrophic_respiration_carbon_flux', model)
        rh_historical_cube_new = open_netCDF(rh_historical_cube_new)
        # Soil Carbon (cSoil)
        cSoil_historical_cube_new = combine_netCDF_cmip5('cmip5_data/cSoil_Lmon_'+model+'_historical*', 'soil_carbon_content', model)
        cSoil_historical_cube_new = open_netCDF(cSoil_historical_cube_new)
        # Near Surface Air Temperature (tas)
        tas_historical_cube = combine_netCDF_cmip5('cmip5_data/tas_Amon_'+model+'_historical*', 'air_temperature', model)
        ...
        # Heterotrophic Respiration (RH)
        rh_cube = combine_netCDF_cmip5('cmip5_data/rh_Lmon_'+model+'_'+rcp+'_*', 'heterotrophic_respiration_carbon_flux', model)
        rh_cube = open_netCDF(rh_cube)
        # Soil Carbon (cSoil)
        cSoil_cube = combine_netCDF_cmip5('cmip5_data/cSoil_Lmon_'+model+'_'+rcp+'_*', 'soil_carbon_content', model)
        cSoil_cube = open_netCDF(cSoil_cube)
        # Near Surface Air Temperature (tas)
        tas_cube = combine_netCDF_cmip5('cmip5_data/tas_Amon_'+model+'_'+rcp+'_*', 'air_temperature', model)
        ...
        landfraction = combine_netCDF_cmip5('cmip5_data/sftlf_fx_'+model+'_historical_r0i0p0.nc', 'land_area_fraction', model)
...
for ssp_option in range(0, ssp_options_length):
    ...
    for model_i in range(0, n_models):
        ...
        # Heterotrophic Respiration (RH)
        rh_historical_cube_new = combine_netCDF_cmip6('cmip6_data/rh_Lmon_'+model+'_historical*', model)
        rh_historical_cube_new = open_netCDF(rh_historical_cube_new)
        # Soil Carbon (cSoil)
        cSoil_historical_cube_new = combine_netCDF_cmip6('cmip6_data/cSoil_Emon_'+model+'_historical*', model)
        cSoil_historical_cube_new = open_netCDF(cSoil_historical_cube_new)
        # Near Surface Air Temperature (tas)
        tas_historical_cube = combine_netCDF_cmip6('cmip6_data/tas_Amon_'+model+'_historical*', model)
        tas_historical_cube = open_netCDF(tas_historical_cube)
        ...
        # Heterotrophic Respiration (RH)
        rh_cube = combine_netCDF_cmip6('cmip6_data/rh_Lmon_'+model+'_'+ssp+'_*', model)
        rh_cube = open_netCDF(rh_cube)
        # Soil Carbon (cSoil)
        cSoil_cube = combine_netCDF_cmip6('cmip6_data/cSoil_Emon_'+model+'_'+ssp+'_*', model)
        cSoil_cube = open_netCDF(cSoil_cube)
        # Near Surface Air Temperature (tas)
        tas_cube = combine_netCDF_cmip6('cmip6_data/tas_Amon_'+model+'_'+ssp+'_*', model)
        tas_cube = open_netCDF(tas_cube)
        # Select future time period
        rh_cube = select_time(rh_cube, 2010, 2100)
        cSoil_cube = select_time(cSoil_cube, 2010, 2100)
        ...
        # convert numpy array to cube
        delta_c_soil_actual_cube = numpy_to_cube(delta_c_soil_actual, cube_save, 3)
        landfraction = combine_netCDF_model('cmip6_data/sftlf_fx_'+model+'_historical*', model)
        actual_delta_cSoil_global = global_total_percentage(delta_c_soil_actual_cube, landfrac=landfraction, latlon_cons=None)
        actual_delta_cSoil_global_data = actual_delta_cSoil_global.data
...
for rcp_option in range(0, rcp_options_length):
    ...
    for model_i in range(0, n_models_cmip5):
        ...
        cSoil_inital_cube = combine_netCDF_cmip5('cmip5_data/cSoil_Lmon_'+model+'_historical*', 'soil_carbon_content', model)
        ...
        landfraction = combine_netCDF_cmip5('cmip5_data/sftlf_fx_'+model+'_*', 'land_area_fraction', model)
...
for ssp_option in range(0, ssp_options_length):
    ...
    for model_i in range(0, n_models):
        ...
        # Calculating global totals of hitorical Soil Carbon (cSoil)
        cSoil_inital_cube = combine_netCDF_cmip6('cmip6_data/cSoil_Emon_'+model+'_historical*', model)
        cSoil_inital_cube = open_netCDF(cSoil_inital_cube)
        cSoil_inital_cube = select_time(cSoil_inital_cube, 1995, 2005)
        cSoil_inital_cube = time_average(cSoil_inital_cube)
        landfraction = combine_netCDF_model('cmip6_data/sftlf_fx_'+model+'_historical*', model)
