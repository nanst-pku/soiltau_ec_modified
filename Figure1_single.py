...
# CMIP6 models
cmip6_models = ['ACCESS-ESM1-5']
model_colours = ['darkblue']
n_models = len(cmip6_models)

# ssp Scenarios
ssp_options = ['ssp126','ssp585']
...
for ssp_option in range(0, ssp_options_length):
    ...
    for model_i in range(0, n_models):
        ...
        delta_c_soil_actual_cube = numpy_to_cube(delta_c_soil_actual, cube_save, 3,'other')
...
for ssp_option in range(0, ssp_options_length):
    ...
    for model_i in range(0, n_models):
        ...
        delta_c_soil_actual = np.ma.masked_invalid(delta_c_soil_actual) # Masking invalid values
        # convert numpy array to cube
        delta_c_soil_actual_cube = numpy_to_cube(delta_c_soil_actual, cube_save, 3,'g')
        landfraction = combine_netCDF_model('cmip6_data/sftlf_fx_'+model+'_historical*', model)
        actual_delta_cSoil_global = global_total_percentage(delta_c_soil_actual_cube, landfrac=landfraction, latlon_cons=None)
        actual_delta_cSoil_global_data = actual_delta_cSoil_global.data
...
fig_figure2.savefig('paper_figures/Figure1_v1.pdf', bbox_inches='tight')
plt.close()