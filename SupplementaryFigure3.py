...
Script produces Supplementary Figure 3 in Varney et al. 2020 Nature Communications
- finding the quadratic fit for tau v T (spatial temperature sensitivity of soil carbon turnover time)
for each CMIP5 model in this study
"""
...
# set up of figure
fig_cmip5 = plt.figure(1, figsize=(18,14))
gs = gspec.GridSpec(3, 3, figure=fig_cmip5, hspace=0.2)
n = 9
column_1 = 0
row_1 = 0
n_columns_1 = 3
...
# CMIP5 Models
cmip5_data_in = ['BNU-ESM', 'CanESM2', 'CESM1-CAM5', 'GFDL-ESM2G', 'GISS-E2-R', 'HadGEM2-ES', 'IPSL-CM5A-LR', 'MIROC-ESM', 'NorESM1-M']
...
for model_i, a, in zip(range(n_models), range(n)):
    ...
    ax = fig_cmip5.add_subplot(gs[row_1, column_1])
    # seleting the models
    model = cmip5_data_in[model_i]
    print(model)


    # Heterotrophic Respiration (RH)
    rh_historical_cube = combine_netCDF_cmip5('cmip5_data/rh_Lmon_'+model+'_historical*', 'heterotrophic_respiration_carbon_flux', model)
    rh_historical_cube = open_netCDF(rh_historical_cube)
    # Soil Carbon (cSoil)
    cSoil_historical_cube = combine_netCDF_cmip5('cmip5_data/cSoil_Lmon_'+model+'_historical*', 'soil_carbon_content', model)
    ...
    ax.title.set_text(str(cmip5_data_in[model_i]))
    ax.set_xlim((-22, 30))
    ax.set_ylim((1, 9.9))
...
fig_cmip5.savefig('paper_figures/SupplementaryFigure3_v1.pdf', bbox_inches='tight')
plt.close()
