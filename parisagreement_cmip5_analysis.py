...
landfraction_obs = combine_netCDF_variable('observational_datasets/luc4c_landmask.nc', 'mask')



# Loading regrid cube
regrid_cube = iris.load_cube('observational_datasets/Tair_WFDEI_ann.nc')
...
for temp_option in range(0, temperature_change_options_length):
    ...
    for rcp_option in range(0, rcp_options_length):
        ...
        for model_i in range(0, n_models):
            ...
            tas_preindustrial_cube = combine_netCDF_cmip5('cmip5_data/tas_Amon_'+model+'_historical*', 'air_temperature', model)
            ...
            tas_cube = combine_netCDF_cmip5('cmip5_historical_'+rcp+'/tas_Amon_'+model+'_*', 'air_temperature', model)
            ...
            rh_cube = combine_netCDF_cmip5('cmip5_historical_'+rcp+'/rh_Lmon_'+model+'_*', 'heterotrophic_respiration_carbon_flux', model)
            rh_cube = open_netCDF(rh_cube)
            cSoil_cube = combine_netCDF_cmip5('cmip5_historical_'+rcp+'/cSoil_Lmon_'+model+'_*', 'soil_carbon_content', model)
            ...
            # model land fraction
            landfraction = combine_netCDF_cmip5('cmip5_data/sftlf_fx_'+model+'*', 'land_area_fraction', model)
