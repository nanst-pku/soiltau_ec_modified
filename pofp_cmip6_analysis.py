...
for ssp_option in range(0, ssp_options_length):
    ...
    for model_i in range(0, n_models):
        ...
        rh_historical_cube = combine_netCDF_rh_cmip6('cmip6_data/rh_Lmon_'+model+'_historical*', model)
        rh_historical_cube = open_netCDF(rh_historical_cube)
        # Soil Carbon (cSoil)
        cSoil_historical_cube = combine_netCDF_cmip6('cmip6_data/cSoil_Emon_'+model+'_historical*', model)
        ...
        # future
        tas_future_cube = combine_netCDF_cmip6('cmip6_data/tas_Amon_'+model+'_'+ssp+'_*', model)
        ...
        # future
        # Heterotrophic Respiration (RH)
        rh_future_cube = combine_netCDF_rh_cmip6('cmip6_data/rh_Lmon_'+model+'_'+ssp+'*', model)
        rh_future_cube = open_netCDF(rh_future_cube)
        # Soil Carbon (cSoil)
        cSoil_future_cube = combine_netCDF_cmip6('cmip6_data/cSoil_Emon_'+model+'_'+ssp+'*', model)
