...
for rcp_option in range(0, rcp_options_length):
    ...
    for model_i in range(0, n_models):
        ...
        # future
        tas_future_cube = combine_netCDF_cmip5('cmip5_data/tas_Amon_'+model+'_'+rcp+'_*', 'air_temperature', model)
        ...
        # future
        # Heterotrophic Respiration (RH)
        rh_future_cube = combine_netCDF_cmip5('cmip5_data/rh_Lmon_'+model+'_'+rcp+'*', 'heterotrophic_respiration_carbon_flux', model)
        rh_future_cube = open_netCDF(rh_future_cube)
        # Soil Carbon (cSoil)
        cSoil_future_cube = combine_netCDF_cmip5('cmip5_data/cSoil_Lmon_'+model+'_'+rcp+'*', 'soil_carbon_content', model)
